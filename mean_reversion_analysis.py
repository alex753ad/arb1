...
ВЕРСИЯ v45 HYBRID | 18.03.2026 | Гибрид: лучшее из v44 + надёжность старой формулы

Дата: 18 марта 2026

ИЗМЕНЕНИЯ v45 (hybrid):
  [HYBRID] Quality Score — гибрид старой и новой (v44) формулы:
    - FDR (25): непрерывная шкала из v44 (убирает скачки ступенек), вес из старой (25)
    - Stability (25): старый вес (был 30 в v44 → вернули 25)
    - Hurst (20): fallback=5 (старый; v44 давал 10, завышая Q для багнутых DFA)
    - ADF (15): старый вес (был 10 в v44 → вернули 15)
    - Hedge ratio (15): зоны из v44 (оптимум ≤3.5), веса из старой (15/10/5)
    - HR uncertainty: градуированный штраф из v44 (-8/-15/-25 вместо одной ступени -10)
    - UBT penalty: оставлен из v44 (штраф за провал upper bound теста)
  Обоснование: симуляция показала, что v44 пропускает убыточные пары (33 сделки, -16.85%)
    из-за инфляции Q через Stability(30) + Hurst fallback(10) + HR(20).
    Гибрид сохраняет стабильность непрерывной FDR и точность градуированного HR unc,
    но возвращает строгую фильтрацию старых весов.

  Предыдущие изменения:
  v10.4: sanitize HR cap 30, DFA min_window=8, HR>10 penalty
  v10.3: Stability gate, Z cutoff 4.5
...
class _HurstFallback(float):
    """BUG-024 FIX: float subclass carrying is_fallback=True.
    Behaves exactly like float 0.5 in arithmetic/comparison,
    but callers can detect it via getattr(h, 'is_fallback', False).
    """
    is_fallback: bool = True


HURST_FALLBACK = _HurstFallback(0.5)  # BUG-024: sentinel for every fallback path
...
def calculate_confidence(hurst, stability_score, fdr_passed, adf_passed,
    ...
    # N-03 FIX: abs(hedge_ratio) — отрицательный HR допустим (BUG-014 FIX)
...
def calculate_quality_score(hurst, ou_params, pvalue_adj, stability_score,
                            ...
                            ubt_passed=None):
    """
    Quality Score v45 HYBRID (0-100) — оценка ПАРЫ, без привязки к текущему Z.

    Гибрид: непрерывность v44 + строгость старой формулы.

    Компоненты (макс 100):
      FDR p-value:    25  — непрерывная шкала (из v44), вес (из старой)
      Stability:      25  — устойчивость во времени (старый вес)
      Hurst (DFA):    20  — подтверждение mean-reversion (fallback=5, как в старой)
      ADF:            15  — независимый тест стационарности (старый вес)
      Hedge ratio:    15  — зоны из v44 (оптимум ≤3.5), веса из старой (15/10/5)
                     ----
                     100
    ...
    Модификаторы:
      ...
      HR uncertainty (градуированный, из v44):
        > 0.7 → -25, > 0.5 → -15, > 0.3 → -8, иначе 0
      UBT fail                 → -5  (провал upper bound теста, из v44)

    Изменения относительно старой формулы:
      + FDR непрерывная: max(0, (0.15-p)/0.15)*25 — убирает скачки при p≈0.05
      + HR uncertainty градуированный: 3 ступени вместо одной
      + HR зоны ужесточены: оптимум ≤3.5 (было ≤5.0)
      + UBT penalty добавлен
      = Остальное идентично старой формуле
    """
    bd = {}

    # ── FDR (25) — непрерывная шкала из v44, вес из старой ──────────────
    # Вместо ступенек 25/20/12/0 → плавная кривая 0-25.
    # Убирает нестабильность: скачок p 0.051→0.049 давал мгновенно +12.
    bd['fdr'] = min(25, int(max(0.0, (0.15 - pvalue_adj) / 0.15) * 25))

    # ── Stability (25) — старый вес ─────────────────────────────────────
    # v44 давал 30 — завышало Q, т.к. Stability сильно коррелирует с FDR.
    bd['stability'] = max(0, min(25, int(stability_score * 25)))

    # ── Hurst (20) — fallback=5 (старый) ───────────────────────────────
    # v44 давал fallback=10, что инфлировало Q для пар с багнутым DFA.
    if hurst_is_fallback or hurst == 0.5:
        bd['hurst'] = 5   # Старое значение; v44 давал 10
    elif hurst <= 0.30:
        bd['hurst'] = 20
    elif hurst <= 0.40:
        bd['hurst'] = 15
    ...
    # ── ADF (15) — старый вес ──────────────────────────────────────────
    # v44 снижал до 10, но ADF — независимый тест, не дублирует FDR
    # (FDR = BH-поправка множественного тестирования, ADF = конкретный спред).
    bd['adf'] = 15 if adf_passed else 0

    # ── Hedge ratio (15) — зоны v44 (оптимум ≤3.5), веса старые ───────
    # v44 ужесточил зоны (≤3.5 вместо ≤5.0) — это правильно,
    # пара с HR=4.5 торгуется хуже, чем с HR=1.5.
    # Но веса 20/12/5 из v44 завышали Q → возвращаем 15/10/5.
    abs_hr = abs(hedge_ratio) if hedge_ratio else 0
    if not hedge_ratio or hedge_ratio <= 0 or abs_hr > 30:  # N-02 FIX: None-safe
        bd['hedge_ratio'] = 0
    elif 0.2 <= abs_hr <= 3.5:
        bd['hedge_ratio'] = 15   # Оптимальная зона (v44: ≤3.5, старая: ≤5.0)
    elif 0.1 <= abs_hr <= 7.0:
        bd['hedge_ratio'] = 10   # Допустимая зона
    elif 0.05 <= abs_hr <= 15.0:
        bd['hedge_ratio'] = 5    # Экстремальная, но торгуемая
    else:
        bd['hedge_ratio'] = 0
    ...
    # ── HR uncertainty modifier (градуированный, из v44) ───────────────
    # Старая формула: >0.5 → -10 (одна ступень).
    # v44: градуированный штраф — точнее отражает неопределённость Калмана.
    # 0.3-0.5 → -8 (раньше не штрафовалось!), >0.5 → -15, >0.7 → -25.
    bd['hr_unc_penalty'] = 0
    ...
    # ── UBT penalty (из v44) ──────────────────────────────────────────
    # Upper Bound Test: если тест не пройден, пара статистически ненадёжна.
    if ubt_passed is not None and not ubt_passed:
        bd['ubt_penalty'] = -5
    else:
        bd['ubt_penalty'] = 0
...
if __name__ == "__main__":
    ...
    hl_bars = ou['halflife_ou'] * 24 / 4.0 if ou else 10  # HL_days → hours → bars (4h TF)

    # 1. Adaptive Robust Z-score
    print(f"\n--- Adaptive Robust Z-Score ---")
    print(f"OU HL = {ou['halflife_ou']:.1f}д ({ou['halflife_ou']*24:.1f}ч) → {hl_bars:.1f} bars (4h TF)")
    ...
    # 5. Quality with crossing penalty + HR unc penalty + UBT
    print(f"\n--- Quality Score v45 HYBRID ---")
    ...
    q4, bd4 = calculate_quality_score(0.2, ou, 0.01, 0.75, 1.5, True, crossing_density=0.08, hr_std=0.1, ubt_passed=False)
    print(f"Active/good HR:       Q={q1} {bd1}")
    print(f"Stuck/good HR:        Q={q2} {bd2}")
    print(f"Active/bad HR(unc):   Q={q3} {bd3}")
    print(f"Active/UBT fail:      Q={q4} {bd4}")
...
# ═══════════════════════════════════════════════════════
# SHIM: Route scoring functions to core.scoring (Волна 2)
# If pairs_scanner.core.scoring is available, use it (tested, fixed).
# Otherwise fall through to local definitions above (backward compat).
# This shim will be removed when mean_reversion_analysis.py is fully split.
# ═══════════════════════════════════════════════════════
try:
    from pairs_scanner.core.scoring import (
        ...
        estimate_exit_time,
        cost_aware_min_z,
        # R-05/R-06 FIX: sanitize_pair и assess_entry_readiness НЕ включены
        # в shim — они зависят от CFG (max_hr_threshold, min_hurst),
        # а core-версии принимают эти значения как параметры с default'ами.
        # Пока callers не передают эти параметры, используем локальные версии
        # которые читают CFG самостоятельно.
    )
except ImportError:
    pass  # Local definitions above remain active
